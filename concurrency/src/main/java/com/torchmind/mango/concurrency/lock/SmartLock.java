...
/**
 * Provides a few extensions to Java's lock interface to improve code quality when working with
 * blocking structures.
 *
...
public interface SmartLock extends Lock {
  ...
  /**
   * Attempts to acquire the lock, execute and action and release the lock while limiting the
   ...
   * @see #unlock() for a more specific documentation on the unlocking process.
   */
  boolean tryExecute(@Nonnegative long time, @Nonnull TimeUnit timeUnit, @Nonnull Runnable runnable)
